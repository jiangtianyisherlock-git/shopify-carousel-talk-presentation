...
const RuntimeStep = ({
  ...
  <article className={`ar-runtime-step ar-kind-${kind} ${className}`}>
      ...
      <div className="ar-board card">
        ...
        <section className={`ar-runtime ${active >= 2 ? "is-visible" : ""}`}>
          ...
          <div className="ar-runtime-flow">
            <RuntimeStep eyebrow="REQUEST" title="用户请求" detail="Shopify URL" />
            <Arrow />
            <RuntimeStep eyebrow="SERVICE" title="Job Service / API" detail="验证 · 创建 job_id · 查询状态" />
            <Arrow />
            <RuntimeStep eyebrow="EXECUTION" title="Orchestrator" detail="计划 · 并行 · 重试 · 回流" />
            <Arrow />
            <RuntimeStep eyebrow="ACTION" kind="action" title="Build / Extract Facts" detail="识别店铺 · 商品 · 媒体" className={active >= 3 ? "is-visible" : ""} />
            <Arrow />
            <RuntimeStep eyebrow="ARTIFACT" kind="artifact" title="Shared Facts" detail="shop · product · media" className={active >= 3 ? "is-visible" : ""} />
            <Arrow label="PARALLEL" />
            <div className={`ar-skill-runs ${active >= 4 ? "is-visible" : ""}`}>
              <span className="ar-invoke mono">INVOKED IN THIS JOB</span>
              <article><b className="mono">RUN A</b><strong>product-to-social-post</strong><p>facts → draft</p></article>
              <article><b className="mono">RUN B</b><strong>product-pic-to-carousel</strong><p>facts + media → plan + manifest</p></article>
              <article><b className="mono">RUN C</b><strong>shopify-to-tiktok-carousel-post</strong><p>product URL → variance + schedule</p></article>
            </div>
            <Arrow />
            <RuntimeStep eyebrow="EXECUTION" title="Assemble & Validate" detail="schema · completeness · validation" className={active >= 5 ? "is-visible" : ""} />
            <Arrow />
            <RuntimeStep eyebrow="ARTIFACT" kind="artifact" title="Delivery Artifact" detail="facts · draft · plan · manifest · validation" className={active >= 5 ? "is-visible" : ""} />
          </div>
        ...
        <section className={`ar-governance ${active >= 5 ? "is-visible" : ""}`}>
          ...
          <div className="ar-governance-flow">
            <article className="ar-mini-artifact"><span className="mono">FROM JOB</span><strong>Delivery Artifact</strong></article>
            <Arrow />
            <article className="ar-human"><i>◉</i><span className="mono">HUMAN</span><strong>Human Review</strong><p>选择 · 编辑 · 确认</p></article>
            <Arrow />
            <article className="ar-gate"><div><span className="mono">DECISION</span><strong>Publish Gate</strong><p>内容 · Bio Link · 授权 · 可访问性</p></div></article>
            <div className="ar-gate-outcomes">
              <b>PASS → SCHEDULE / PUBLISH</b>
